...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        queires = torch.FloatTensor(queries)
        keys = torch.FloatTensor(keys)
        values = torch.FloatTensor(values)
        # Scaled Dot Product
        self._scores = torch.bmm(queries.unsqueeze(0), keys.unsqueeze(0).transpose(1, 2)) / np.sqrt(K)
        ...
        attention = torch.bmm(self._scores, values.unsqueeze(0))
        attention = torch.FloatTensor(attention)

        # Concatenat the heads
        attention_heads = torch.cat(attention.chunk(self._h, dim=0), dim=-1)
        attention_heads = torch.FloatTensor(attention_heads)
